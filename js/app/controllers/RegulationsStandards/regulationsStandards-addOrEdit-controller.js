define([
  ...
  app.controller('regulationsStandards-addOrEdit-controller', [
    '$stateParams',
    '$rootScope',
    '$scope',
    '$state',
    ...
    function (
      ...
      var initialize = function () {
        $scope.isLawMaintain = utils.getListItem(
          '法规发布维护',
          'menuname',
          user.menus
        );
        ...
        regulationService.SelectLawstandardType(function (params) {
          ...
          if (postData.type == 'edit') {
            ...
            regulationService.getLawstandardById(
              postData.item.id,
              function (params) {
                statusBegin = params.approvestatus;
                ...
                if ($scope.data.releasedate) {
                  $scope.data.releasedate = utils.parseTime(
                    new Date($scope.data.releasedate),
                    'YYYY-MM-DD'
                  );
                }
                if ($scope.data.impdate) {
                  $scope.data.impdate = utils.parseTime(
                    new Date($scope.data.impdate),
                    'YYYY-MM-DD'
                  );
                }
                angular.element('.nicEdit-main')[0].innerHTML =
                  $scope.data.summaryinfo;
                //发布部门
                if ($scope.data.organizationList) {
                  for (
                    var i = 0;
                    i < $scope.data.organizationList.length;
                    i++
                  ) {
                    $scope.organization.push({
                      id: $scope.data.organizationList[i],
                    });
                  }
                } else {
                  $scope.organization = [];
      ...
      var define_function = function () {
        ...
        $scope.save = function (params) {
          systemService.getApproveSetting(function (res) {
            ...
            regulationService.SaveOrUpdateLawstandard(
              $scope.data,
              function (response) {
                if (response == 200) {
                  toaster.pop({
                  ...
                  if (
                    $scope.data.approvestatus == 2 ||
                    ($scope.data.approvestatus == 3 && statusBegin != 3)
                  ) {
