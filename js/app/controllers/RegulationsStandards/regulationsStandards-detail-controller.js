define([
  ...
  app.controller('regulationsStandards-detail-controller', [
    '$stateParams',
    '$rootScope',
    '$scope',
    '$state',
    ...
    function (
      ...
      var initialize = function () {
        ...
        $scope.isDownload = utils.getListItem(
          '法规标准下载',
          'menuname',
          user.menus
        );

        $scope.isTop = utils.getListItem(
          '法规标准置顶',
          'menuname',
          user.menus
        );
        ...
        regulationService.getDetailLawstandardById(
          postData.item.id,
          function (params) {
            $scope.DetaiData = params;
            $scope.DetaiData.inputdate = utils.parseTime(
              new Date($scope.DetaiData.inputdate),
              'YYYY-MM-DD'
            );
            if ($scope.DetaiData.modifydate) {
              $scope.DetaiData.modifydate = utils.parseTime(
                new Date($scope.DetaiData.modifydate),
                'YYYY-MM-DD'
              );
            }
          }
      ...
      var define_function = function () {
        $scope.goState = function () {
          ...
          if ($scope.histroyData.length == 1) {
            ...
            if (
              postData.clickValue == 'approve' ||
              postData.clickValue == 'fav'
            ) {
            ...
            regulationService.getDetailLawstandardById(
              $scope.histroyData[$scope.histroyData.length - 1],
              function (res) {
                $scope.DetaiData = res;
                $scope.DetaiData.inputdate = utils.parseTime(
                  new Date($scope.DetaiData.inputdate),
