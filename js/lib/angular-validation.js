  ...
  function Validator($injector) {
    ...
    var invalidFunc = function(element, validMessage, validation, scope, ctrl, attrs) {
      ...
      if (element.attr('no-validation-message')) {
        ...
        var tempHtml = $compile($validationProvider.getErrorHTML(messageToShow, element, attrs))(scope);
        if(validation=='lessOrEqual'||validation=='moreOrEqual') 
          tempHtml[0].innerHTML = tempHtml[0].innerHTML.replace('replaceText',element[0].attributes.compareTarget.nodeValue);
        messageElem.html('').append($(tempHtml));
        messageElem.css('display', '');
      } else {
        messageElem.css('display', 'none');
      }
    ...
    var checkValidation = function(scope, element, attrs, ctrl, validation, value) {
      ...
      var validationGroup = attrs.validationGroup;
      var validateGroup = attrs.validateGroup;
      var valid = {
        success: function(message) {
          ...
          if (leftValidation.length) {
            ...
            var temp = angular.element('*[validate-group="'+validateGroup+'"]');
            var othr;
            angular.forEach(temp,function(v,k){
              v = $(v);
              if(v[0].name!=element[0].name){
                v.next().css('display','none');
                v.removeClass('ng-invalid');
              }
            });
          }
          else {
        ...
        error: function(message) {
          if(validateGroup){
            ...
            angular.forEach(temp,function(v,k){
              v = $(v);
              if(v[0].name!=element[0].name){
                if(v.next()[0].innerHTML){
                  v.next().css('display','');
                  v.addClass('ng-invalid');
                }
              }
            });
          }
