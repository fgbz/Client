define([
  ...
  return declare([GraphicsLayer], {
    constructor: function(options) {
      ...
      // this._oboaSymbol= options.oboaSymbol;
      // this._ofoaSymbol= options.ofoaSymbol;
    ...
    onClick: function(e) {
      ...
      if ( singles.length > this._maxSingles ) {
        // if(esriAlert){
        //   esriAlert("当前聚合数量超过 " + this._maxSingles + " 个建筑物. 请放大比例再查看详情.");
        // }else{
        //   alert("当前聚合数量超过 " + this._maxSingles + " 个建筑物. 请放大比例再查看详情.")
        // }
        // ;
    ...
    _showAllClusters: function() {
      ...
      document.getElementById('buildingsloading').style.display='none';
    },
    ...
    _addSingles: function(singles) {
      ...
      arrayUtils.forEach(singles, function(p) {
        ...
        //cluster extend support for polygon by haoranx
        var geometry,symbol;
        if(p.attributes.geometryStr){
          var geometryJson = JSON.parse(p.attributes.geometryStr);
          geometry = geometryJsonUtils.fromJson(geometryJson);
          symbol = new SimpleFillSymbol(SimpleFillSymbol.STYLE_SOLID,
                                new SimpleLineSymbol(SimpleLineSymbol.STYLE_SOLID, new Color([30, 74, 164, 0.25]), 2),
                                new Color([30, 74, 164, 0.75]))
        }else{
          geometry=new Point(p.x, p.y, this._sr);
          // if(p.attributes.Type=='一户一档'){
          //   symbol=this._ofoaSymbol;
          // }else if(p.attributes.Type=='一楼一档'){
          //   symbol=this._oboaSymbol;
          // }
